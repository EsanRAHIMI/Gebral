...
router.post("/", authenticateToken, async (req, res) => {
  ...
  const userId = String(req.user.id); // اطمینان از رشته بودن user_id

  try {
    // In the POST route handler for /tasks:
    const result = await pool.query(
      `INSERT INTO tasks (title, description, status, user_id, "order") 
   VALUES ($1, $2, $3, $4, 
   (SELECT COALESCE(MAX("order"), 0) + 1 FROM tasks WHERE user_id = $4::VARCHAR(50))) 
   RETURNING *`,
...
router.get("/", authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, title, description, status, "order", created_at FROM tasks WHERE user_id = $1 ORDER BY "order" ASC',
      [userId]
    );
